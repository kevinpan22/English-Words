...
// ========== 课本单元 ==========
let selectedGrade = '7年级';
let selectedTerm  = '7A';
let selectedUnit  = null;

function getTermsForGrade(grade) {
    const g = grade.replace('年级', '');
    return [`${g}A`, `${g}B`];
}
...
function selectGrade(grade) {
    selectedGrade = grade;
    selectedTerm  = getTermsForGrade(grade)[0]; // 自动选第一学期
    selectedUnit  = null;
    refreshGradeTab();
    refreshTermTab();
    renderUnitList();
...
function refreshTermTab() {
    const termTab = document.getElementById('termTab');
    const terms = getTermsForGrade(selectedGrade);
    termTab.innerHTML = terms.map(t =>
        `<button class="cur-tab${t === selectedTerm ? ' active' : ''}" onclick="selectTerm('${t}')">${t}</button>`
    ).join('');
}
